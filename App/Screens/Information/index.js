...
class Information extends Component {
    ...
    isFormFilled() {
      ...
      let check_Address = Validations.checkPassword(this.state.Address);
      let check_Area = Validations.checkPassword(this.state.Area);
    ...
    async placeBuyerOrder(){
      ...
      const data={
          seller:  Array.isArray(this.state.data.seller)  ?  this.state.data.seller : [this.state.data.seller],
          product:  Array.isArray(this.state.data.product)  ?  this.state.data.product : [this.state.data.product], 
          quantity:  Array.isArray(this.state.data.quantity)  ?  this.state.data.quantity : [Number(this.state.data.quantity)],
    ...
    return (
      <View style={styles.wrapperView}>
        ...
        <View style={styles.headerView}>
            ...
            <Image 
            style={{width: 25, height: 15, marginRight: isIphoneXorAbove ? 60 : 50, resizeMode:'contain'}}
            source={require('../../assets/backblack.png')}/>
            </TouchableOpacity>
            <Text style={{fontSize: 24, fontWeight: '600', color: BLACK.dark}}>Profile Information</Text>
          </View>
          <View style={{flex:1,width: SCREEN.width - 40, alignSelf:'center'}}>
            ...
            />
            <Image style={{width: 25, height:25, position:'absolute', bottom:35, left: 10}} source={require('../../assets/phone.png')}/> 
            </View>
            ...
            style={styles.TextInput}
            />
            <Image style={{width: 25, height:18, position:'absolute', bottom:35, left: 10}} source={require('../../assets/mail.png')}/> 
            </View>
            <View style={{borderBottomWidth: 1, borderBottomColor: '#4CAF50', paddingBottom: 20}}>
            <Text style={{fontSize: 22,}}>Shipping address</Text>
            </View>
            <Text style={{fontSize: 22, marginTop: 20}}>Enter Your Address</Text>
            <TextInput
              placeholder='Address'
